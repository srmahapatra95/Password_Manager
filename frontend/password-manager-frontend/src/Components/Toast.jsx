...
function Toast({id, type,message}){
    ...
    return (
        <div className={`w-full flex flex-col justify-between rounded-md text-slate-200 bg-gray-800 border-2 border-slate-200 ${dismissed?"toasts-dismissed":""}`} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
            <div className='w-full flex flex-row items-center justify-between'>
                <div className='flex flex-row justify-start items-center'>
                    <div className='p-3 rounded-md'>{icon}</div>
                    <p className="text-slate-200 text-center font-mono font-bold">{message}</p>
                </div>
                <button id="close-screen-btn" onClick={handleToastClose} className=" w-4 h-4 m-2 text-white hover:bg-rose-50 hover:text-black rounded-full cursor-pointer">
